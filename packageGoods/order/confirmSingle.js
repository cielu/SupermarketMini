...
_core["default"].page({
  ...
  methods: _objectSpread({}, (0, _x.mapActions)(['syncCurrentArea']), {
    ...
    formatTime: function formatTime() {
      ...
      if (typeof bizHours !== 'undefined') {
        // 获取营业时间的小时与分钟
        bizStart = bizHours[0].split(':');

        if (curHour < bizStart[0]) {
          curHour = parseInt(bizStart[0]);
          curMin = parseInt(bizStart[1]);
        }

        if (bizHours.length === 2) {
          bizEnd = bizHours[1].split(':');
        }
      }
    ...
    onPlusGoodsNum: function onPlusGoodsNum() {
      ...
      if (this.form.type === 'group') {
        ...
        price = this.goods.discountPrice > 0 ? this.goods.discountPrice : this.goods.salePrice;
      } // 实付价


      this.settle.subtotalPrice = this.form.goodsNum * price;
      this.settle.totalPrice = this.settle.subtotalPrice;
    },
          ...
          while (1) {
            switch (_context3.prev = _context3.next) {
              case 0:
                // 传入参数
                if (this.currentArea !== null) {
                  this.form.lat = this.currentArea.location.lat;
                  this.form.lng = this.currentArea.location.lng;
                } // get data


                _context3.next = 3;
                return (0, _order.confirmOrder)(this.form);
              ...
              case 3:
                ...
                this.form.addressId = this.address.addressId; // 团购订单

                if (this.form.type === 'group') {
                  this.form.goodsNum = this.goods.saleStartNum;
                  this.settle.subtotalPrice = (this.goods.groupPrice * this.form.goodsNum).toFixed(2);
                  this.settle.totalPrice = this.settle.subtotalPrice;
                } else {
                  this.form.goodsNum = this.goods.saleStartNum;
                  price = this.goods.discountPrice > 0 ? this.goods.discountPrice : this.goods.salePrice;
                  this.settle.subtotalPrice = (parseFloat(price) * this.form.goodsNum).toFixed(2);
                  this.settle.totalPrice = this.settle.subtotalPrice;
                }

                if (this.goods === null) {
                  _util["default"].toast('商品不存在，请重新下单'); // this.onClickBack(); return
...
}, {info: {"components":{"van-nav-bar":{"path":"../../$vendor/@vant/weapp/dist/nav-bar/index"},"van-icon":{"path":"../../$vendor/@vant/weapp/dist/icon/index"},"van-loading":{"path":"../../$vendor/@vant/weapp/dist/loading/index"},"van-stepper":{"path":"../../$vendor/@vant/weapp/dist/stepper/index"},"van-popup":{"path":"../../$vendor/@vant/weapp/dist/popup/index"},"van-cell":{"path":"../../$vendor/@vant/weapp/dist/cell/index"}},"on":{"31-0":["clickLeft"],"31-3":["minus","plus"],"31-7":["close"],"31-9":["tap"],"31-12":["touchmove","close"],"31-15":["tap"],"31-18":["tap"]}}, handlers: {'31-0': {"clickLeft": function proxy () {
  ...
  }},'31-11': {"tap": function proxy (index) {
  ...
  }},'31-12': {"touchmove": function proxy () {
    var $event = arguments[arguments.length - 1];
    var _vm=this;
      return (function () {
        return($event);
  ...
  }},'31-14': {"touchmove": function proxy () {
    var $event = arguments[arguments.length - 1];
    var _vm=this;
      return (function () {
        return($event);
  ...
  }},'31-17': {"tap": function proxy (index) {
